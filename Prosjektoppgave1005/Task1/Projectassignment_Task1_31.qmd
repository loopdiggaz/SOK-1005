...
df <- df %>% # Relocateing for tidiness. 
  ...
  relocate(gm, .before = grocery) %>% 
  relocate(gmcoup, .before = grocery) %>% 
  relocate(custcoun, .before = grocery)
...
df <- df[, 1:113] # Picking columns i need. 

df <- df[,-33:-74] # Deleting columns i dont need. 
df <- df[,-18:-30]
```

```{r}
write_csv(df, "df_final.csv") # Writing final df as csv file for sales report. 
```
